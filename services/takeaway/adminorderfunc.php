...
echo "<br><a><b>您好：".$_SESSION['username']." 以下是您的订单</b><br><br> [本页每30秒自动刷新]</a><br/>";
				...
				//Find Order Number
				$sqloid = "select oid from orderlist";
				...
				//Get all order
				$sqlgeto = "select * from orderlist order by unixtime DESC limit $offset,$show_num";
					...
					echo "<tr>";
						...
						switch ($orderstatus){
							...
							case 0;
								echo "	<td>
											<form action = './services/takeaway/orderupdate.php' method='post'>
												<a>订单被撤销（0）</a>
												<input type='submit' class='doyes' name='update' value = '撤销订单(0)'>
												<input type='submit' class='doyes' name='update' value = '送出订单(22)'>
												<input type='submit' class='doyes' name='update' value = '结束订单(99)'>
												<input type='submit' class='dono' name='update' value = '删除订单'>
							...
							case 111;
								echo "	<td>
											<form action = './services/takeaway/orderupdate.php' method='post'>
												<a>订单被店家拒绝，理由是：".$row['refuseReason']."（111）</a>
												<input type='submit' class='doyes' name='update' value = '撤销订单(0)'>
												<input type='submit' class='doyes' name='update' value = '送出订单(22)'>
												<input type='submit' class='doyes' name='update' value = '结束订单(99)'>
												<input type='submit' class='dono' name='update' value = '删除订单'>
							...
							case 55;
								echo "	<td>
											<form action = './services/takeaway/orderupdate.php' method='post'>
												<a>店家申请撤单,理由：".$row['refuseReason']."（55）</a><br>
												<input type='submit' class='doyes' name='update' value = '撤销订单(0)'>
												<input type='submit' class='doyes' name='update' value = '送出订单(22)'>
												<input type='submit' class='doyes' name='update' value = '结束订单(99)'>
												<input type='submit' class='dono' name='update' value = '删除订单'>
